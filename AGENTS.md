> [!IMPORTANT]
> **아래 규칙을 기본 원칙으로 준수하며, 상위 지시 또는 이 문서에 명시된 예외가 있으면 해당 지시를 우선한다.**
...
- Git 사용자 이름과 GitHub 제공 `noreply` 이메일은 사용자의 명시적인 지시에 따라 로컬 `.git/config`에 저장할 수 있다.
- 이 예외는 개인 이메일이나 다른 민감 정보에는 적용하지 않는다.
...
- `main` branch에서는 작업 파일을 직접 수정하거나 `commit`하지 않는다.
- 원격 변경을 동기화하기 위한 fast-forward 방식의 update는 허용한다.
- 작업 branch를 생성하기 전에 미커밋 변경이 있는지 확인하고, 기존 변경이 있으면 임의로 이동하거나 포함하지 않는다.
- 원격 저장소가 있으면 `main`을 fast-forward 방식으로 동기화한 후 작업 branch를 생성한다.
- 동기화할 수 없으면 작업을 중단하고 사용자에게 알린다.
- 하나의 논리적인 변경 목적마다 최신 `main`에서 작업 branch를 생성한다.
...
- merge 전에 Pull Request가 merge 가능한 상태이고 필수 status check가 모두 통과했는지 확인한다.
- 충돌이나 실패한 check가 있으면 merge하지 않고 사용자에게 알린다.
- Pull Request는 기본적으로 `rebase` 방식으로 merge한다.
- 다른 merge 방식이 필요하면 이유를 설명하고 사용자의 명시적인 지시를 받는다.
- 각 작업 branch가 `main`에 merge되면 해당 branch의 로컬과 원격 삭제를 사용자에게 제안한다.
- 작업 branch를 삭제하기 전에 해당 Pull Request가 `main`에 merge되었는지 확인한다.
